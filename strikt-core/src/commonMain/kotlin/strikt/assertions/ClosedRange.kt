 ...
 */
val <T : ClosedRange<E>, E : Comparable<E>> Builder<T>.start: Builder<E>
  get() = get { start }

/**
 * Maps an assertion on the [ClosedRange] to an assertion on its [ClosedRange.endInclusive].
 */
val <T : ClosedRange<E>, E : Comparable<E>> Builder<T>.endInclusive: Builder<E>
  get() = get { endInclusive }

/**
 * Asserts that the subject range contains the provided [element].
 */
infix fun <T : ClosedRange<E>, E : Comparable<E>> Builder<T>.contains(element: E): Builder<T> =
 ...
 */
fun <T : ClosedRange<E>, E : Comparable<E>> Builder<T>.isEmpty(): Builder<T> =
  assert("is empty") {
    when {
      it.isEmpty() -> pass()
      else -> fail()
